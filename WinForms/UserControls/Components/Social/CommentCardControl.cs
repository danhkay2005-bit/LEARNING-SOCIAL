...
namespace WinForms.UserControls.Components.Social
{
    ...
    public partial class CommentCardControl : UserControl
    {
        ...
        private void UpdateReactionDisplay()
        {
            ...
            if (_comment.SoLuotReactions > 0) { lblReactionCount.Text = $"?? {_comment.SoLuotReactions} người đã thả cảm xúc"; lblReactionCount.Visible = true; } else { lblReactionCount.Visible = false; }
        }
        ...
        private void ShowReactionPicker()
        {
            ...
            // ✅ Dùng Unicode emoji với Segoe UI Emoji font
            var reactions = new[] 
            { 
                (LoaiReactionEnum.Thich, "\uD83D\uDC4D", "Like"),      // 👍
                (LoaiReactionEnum.YeuThich, "\u2764\uFE0F", "Love"),   // ❤️
